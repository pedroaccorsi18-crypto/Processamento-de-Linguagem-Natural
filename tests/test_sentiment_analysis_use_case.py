...
from synapse_ai.application.analysis import SentimentAnalysisCommand, SentimentAnalysisUseCase
from synapse_ai.application.analysis.sentiment_analysis import (
    SENTIMENT_ANALYSIS_QUERY,
    SENTIMENT_ANALYSIS_SOURCE_LIMIT,
)
...
def test_sentiment_analysis_use_case_generates_report_successfully() -> None:
    deps = FakeDependencies()
    result = _build_use_case(deps).execute(_command(save_to_history=False))

    assert result.success is True
    assert result.value is not None
    assert result.value.report.overall_sentiment == "Misto"
    ...
    assert deps.retriever.calls[0]["query"] == SENTIMENT_ANALYSIS_QUERY
    assert deps.retriever.calls[0]["limit"] == SENTIMENT_ANALYSIS_SOURCE_LIMIT
    assert deps.retriever.calls[0]["selected_document_ids"] == ["doc-1"]


def test_sentiment_analysis_use_case_returns_info_when_sources_are_missing() -> None:
...
def test_sentiment_analysis_use_case_returns_error_for_known_retrieval_errors() -> None:
...
def test_sentiment_analysis_use_case_returns_error_for_known_generation_errors() -> None:
    deps = FakeDependencies(generation_fail=True)
    result = _build_use_case(deps).execute(_command())

    assert result.success is False
    assert result.severity == ResultSeverity.ERROR
    assert result.message == "Não foi possível gerar a análise de sentimentos organizacionais."


def test_sentiment_analysis_use_case_returns_error_for_known_analysis_errors() -> None:
...
def test_sentiment_analysis_use_case_saves_when_requested() -> None:
    deps = FakeDependencies()
...
def test_sentiment_analysis_use_case_keeps_report_when_persistence_fails() -> None:
    ...
    assert result.value.report.signals[0].dimension == "Urgência"


def _build_use_case(deps: FakeDependencies) -> SentimentAnalysisUseCase:
    return SentimentAnalysisUseCase(
        semantic_retriever=deps.retriever,
        sentiment_report_generator=deps.generate_sentiment_report,
        sentiment_report_saver=deps.save_sentiment_report,
    )


def _command(save_to_history: bool = False) -> SentimentAnalysisCommand:
    return SentimentAnalysisCommand(
...
def _source() -> SourceSnippet:
    return SourceSnippet(
        ...
        filename="ata.pdf",
        chunk_index=0,
        content="Há urgência e tensão na comunicação.",
        similarity=0.91,
    )


def _report(sources: list[SourceSnippet]) -> SentimentReport:
    return SentimentReport(
        overall_sentiment="Misto",
        executive_summary="Há sinais de urgência e tensão.",
        risk_level="Médio",
